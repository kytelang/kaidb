...
pub const WorkloadE = struct {
    ...
    pub const Config = struct {
        ...
        scan_length: usize = 10,
    };

    pub fn init(allocator: Allocator, io: Io, client: *Client, store_name: []const u8, config: Config) !WorkloadE {
    ...
    pub fn run(self: *WorkloadE) !void {
        ...
        std.debug.print("Operation mix: 95% scans, 5% inserts\n", .{});
        std.debug.print("Operations: {d}\n", .{self.config.operation_count});
        std.debug.print("Distribution: Zipfian (Scans)\n\n", .{});
    ...
    fn executeScan(self: *WorkloadE) !bool {
        ...
        const start_key = self.keys.items[start_idx];

        var key_buf: [32]u8 = undefined;
        _ = try std.fmt.bufPrint(&key_buf, "{x:0>32}", .{start_key});

        const select_sql = try std.fmt.allocPrint(self.allocator,
            "SELECT id, field0 FROM {s} WHERE id >= '{s}' LIMIT {d}",
            .{self.store_name, key_buf, self.scan_length}
        );
    ...
    fn executeInsert(self: *WorkloadE) !bool {
        const new_key_val = self.keys.items.len;
        var key_buf: [32]u8 = undefined;
        _ = try std.fmt.bufPrint(&key_buf, "{x:0>32}", .{new_key_val});
        ...
        const reply = self.client.execute(insert_sql) catch |err| {
