...
pub const WorkloadA = struct {
    ...
    pub fn init(allocator: Allocator, io: Io, client: *Client, store_name: []const u8, config: Config) !WorkloadA {
        ...
        const op_mix = operation_chooser.OperationMix.workloadA();
    ...
    pub fn deinit(self: *WorkloadA) void {
    ...
    pub fn load(self: *WorkloadA) !void {
        std.debug.print("\n=== Workload A: Loading Phase ===\n", .{});
    ...
    pub fn run(self: *WorkloadA) !void {
        std.debug.print("\n=== Workload A: Transaction Phase ===\n", .{});
        std.debug.print("Operation mix: 50% reads, 50% updates\n", .{});
    ...
    fn runWarmup(self: *WorkloadA) !void {
    ...
    fn executeRead(self: *WorkloadA) !bool {
    ...
    fn executeUpdate(self: *WorkloadA) !bool {
        if (self.keys.items.len == 0) return false;
        const key_idx = self.key_distribution.next() % self.keys.items.len;
        const doc_key = self.keys.items[key_idx];
        ...
        const reply = self.client.execute(update_sql) catch |err| {
    ...
    fn printResults(self: *WorkloadA, total_time_ms: i64) !void {
        std.debug.print("\n============================================================\n", .{});
        std.debug.print("Workload A Results\n", .{});
        std.debug.print("============================================================\n\n", .{});
        ...
        std.debug.print("\n", .{});
        try self.metrics_tracker.printOperationStats(.update);
