...
pub const WorkloadF = struct {
    ...
    pub fn init(allocator: Allocator, io: Io, client: *Client, store_name: []const u8, config: Config) !WorkloadF {
        ...
        const op_mix = operation_chooser.OperationMix.workloadF();
    ...
    pub fn deinit(self: *WorkloadF) void {
    ...
    pub fn load(self: *WorkloadF) !void {
        std.debug.print("\n=== Workload F: Loading Phase ===\n", .{});
    ...
    pub fn run(self: *WorkloadF) !void {
        std.debug.print("\n=== Workload F: Transaction Phase ===\n", .{});
        std.debug.print("Operation mix: 50% reads, 50% Read-Modify-Write\n", .{});
        std.debug.print("Operations: {d}\n", .{self.config.operation_count});
        std.debug.print("Distribution: Zipfian (RMW)\n\n", .{});
        ...
        while (completed < self.config.operation_count) : (completed += 1) {
            ...
            const result = switch (op_type) {
                ...
                .read_modify_write => try self.executeReadModifyWrite(),
                else => unreachable,
            };
    ...
    fn runWarmup(self: *WorkloadF) !void {
    ...
    fn executeRead(self: *WorkloadF) !bool {
    ...
    fn executeReadModifyWrite(self: *WorkloadF) !bool {
        if (self.keys.items.len == 0) return false;
        const key_idx = self.key_distribution.next() % self.keys.items.len;
        const doc_key = self.keys.items[key_idx];
        ...
        const read_reply = self.client.execute(select_sql) catch |err| {
            std.debug.print("RMW Read error: {}\n", .{err});
            return false;
        };
        self.allocator.free(read_reply);

        const update_sql = try std.fmt.allocPrint(self.allocator,
            "UPDATE {s} SET field0 = '{s}', field1 = '{s}', field2 = '{s}', field3 = '{s}', field4 = '{s}', field5 = '{s}', field6 = '{s}', field7 = '{s}', field8 = '{s}', field9 = '{s}' WHERE id = '{s}'",
            .{self.store_name, self.field_data, self.field_data, self.field_data, self.field_data, self.field_data, self.field_data, self.field_data, self.field_data, self.field_data, self.field_data, key_buf}
        );
        defer self.allocator.free(update_sql);

        const write_reply = self.client.execute(update_sql) catch |err| {
            std.debug.print("RMW Update error: {}\n", .{err});
            return false;
        };
        self.allocator.free(write_reply);
        return true;
    }

    fn printResults(self: *WorkloadF, total_time_ms: i64) !void {
        std.debug.print("\n============================================================\n", .{});
        std.debug.print("Workload F Results\n", .{});
        std.debug.print("============================================================\n\n", .{});
        ...
        std.debug.print("\n", .{});
        try self.metrics_tracker.printOperationStats(.read_modify_write);
