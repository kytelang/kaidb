...
pub const WorkloadB = struct {
    ...
    pub fn init(allocator: Allocator, io: Io, client: *Client, store_name: []const u8, config: Config) !WorkloadB {
        ...
        const op_mix = operation_chooser.OperationMix.workloadB();
    ...
    pub fn deinit(self: *WorkloadB) void {
    ...
    pub fn load(self: *WorkloadB) !void {
        std.debug.print("\n=== Workload B: Loading Phase ===\n", .{});
    ...
    pub fn run(self: *WorkloadB) !void {
        std.debug.print("\n=== Workload B: Transaction Phase ===\n", .{});
        std.debug.print("Operation mix: 95% reads, 5% updates\n", .{});
    ...
    fn runWarmup(self: *WorkloadB) !void {
    ...
    fn executeRead(self: *WorkloadB) !bool {
    ...
    fn executeUpdate(self: *WorkloadB) !bool {
        if (self.keys.items.len == 0) return false;
        const key_idx = self.key_distribution.next() % self.keys.items.len;
        const doc_key = self.keys.items[key_idx];
    ...
    fn printResults(self: *WorkloadB, total_time_ms: i64) !void {
        std.debug.print("\n============================================================\n", .{});
        std.debug.print("Workload B Results\n", .{});
        std.debug.print("============================================================\n\n", .{});
