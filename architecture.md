...
### Measured performance envelope (1M-row orders workload, warm, single machine)

kaidb is **parity-to-2x of PostgreSQL and ahead of MySQL/InnoDB** through an identical client. It wins or ties roughly 13 of 18 queries: the aggregate / count / DISTINCT family, the primary-key and endpoint queries, deep pagination, and the load (via driver pipelining). PostgreSQL wins the medium-selectivity secondary-index range scans (the clustered double-lookup, section 6). This is a genuinely competitive general-purpose result at this scale, not a niche one.
