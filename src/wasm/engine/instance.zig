...
pub const Instance = struct {
    ...
    // Per-call instruction budget (kaidb fuel metering, embed-wasm.md section 5). Set before
    // invoke(); the VM traps with error.FuelExhausted when spent. Defaults to unlimited so
    // callers that do not meter are unaffected.
    fuel_budget: u64 = std.math.maxInt(u64),
    ...
    // Clamp every memory's growth ceiling to `max_pages` (kaidb hard memory cap,
    // embed-wasm.md section 5.3). This is independent of what the module declared, so a
    // module that declares no maximum cannot grow to the full 4 GiB. kaidb calls this right
    // after instantiate() with its per-call policy.
    pub fn limitMemoryPages(self: *Instance, max_pages: u32) !void {
        var i: usize = 0;
        while (i < self.memaddrs.items.len) : (i += 1) {
            const mem_ptr = try self.getMemory(i);
            const current = mem_ptr.max orelse std.math.maxInt(u32);
            mem_ptr.max = @min(current, max_pages);
        }
    }
