...
pub const VirtualMachine = struct {
    ...
    // Fuel metering (kaidb, embed-wasm.md section 5). Every instruction costs one unit;
    // when fuel reaches zero the VM traps with error.FuelExhausted. Defaults to "unlimited"
    // so unmetered callers are unaffected; kaidb sets a per-call budget before invoking.
    fuel: u64 = std.math.maxInt(u64),
    ...
    inline fn dispatch(self: *VirtualMachine, next_ip: usize, code: []Rr) WasmError!void {
        ...
        // Fuel: charge one unit per instruction at the single dispatch chokepoint, then trap
        // when exhausted. The check stays before the tail call so the call remains in tail
        // position (embed-wasm.md section 5).
        if (self.fuel == 0) return error.FuelExhausted;
        self.fuel -= 1;
    ...
    pub fn pushOperand(self: *VirtualMachine, comptime T: type, value: T) !void {
        ...
        self.op_stack[self.op_ptr - 1] = switch (T) {
            ...
            f32 => canonBitsF32(@as(u32, @bitCast(value))),
            f64 => canonBitsF64(@as(u64, @bitCast(value))),
            u32 => @as(u64, value),
            u64 => value,
    ...
    // Canonical-NaN mode (kaidb determinism, embed-wasm.md section 7). WebAssembly permits a
    // NaN result to carry an implementation-defined payload, which lets two hosts (or a primary
    // and a replica) compute bit-different floats from the same inputs. We force every NaN put
    // on the operand stack to the canonical quiet NaN, so float results are bit-identical
    // everywhere. This is the same "canonicalize NaN" determinism mode mature runtimes offer.
    // A float is NaN iff its exponent is all ones and the mantissa is non-zero, i.e. the
    // magnitude bits exceed the infinity encoding.
    inline fn canonBitsF32(bits: u32) u32 {
        return if ((bits & 0x7FFF_FFFF) > 0x7F80_0000) 0x7FC0_0000 else bits;
    }
    inline fn canonBitsF64(bits: u64) u64 {
        return if ((bits & 0x7FFF_FFFF_FFFF_FFFF) > 0x7FF0_0000_0000_0000) 0x7FF8_0000_0000_0000 else bits;
    }
