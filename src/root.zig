...
test "wasm generated column via SQL: computed at insert, stored durably, survives restart (M4)" {
    const allocator = std.testing.allocator;
    var threaded = std.Io.Threaded.init(allocator, .{});
    defer threaded.deinit();
    const io = threaded.io();

    const db_path = "test_wasm_gencol.db";
    defer Io.Dir.deleteFile(.cwd(), io, db_path) catch {};
    defer Io.Dir.deleteTree(.cwd(), io, "udf") catch {};
    ...
    // DBL(x) doubles its i64 argument (the same scalar UDF used by the M1 test).
    const wasm = @embedFile("query/testdata_udf.wasm");
    const hex = comptime std.fmt.bytesToHex(wasm, .lower);
    const create_fn = "CREATE FUNCTION DBL LANGUAGE wasm AS '" ++ hex ++ "'";
    ...
    // Session 1: register the UDF, define a table whose `dbl` column is generated by DBL(x), and
    // insert rows. The value is computed at insert time and written into the ordinary column.
    {
        var db = try Database.open(allocator, io, db_path, 64, null);
        defer db.close();
        var exec = QueryExecutor.init(allocator, db);
        defer exec.deinit();
        try h.q(&exec, allocator, create_fn);
        try h.q(&exec, allocator, "CREATE TABLE g (x INT PRIMARY KEY, dbl INT AS DBL(x))");
        try h.q(&exec, allocator, "INSERT INTO g (x) VALUES (21)");
        try h.q(&exec, allocator, "INSERT INTO g (x) VALUES (5)");

        // The generated column reads back the wasm result: DBL(21)=42, DBL(5)=10. This proves the
        // value was computed at insert time and stored as an ordinary column (not recomputed on
        // read).
        const v21 = try h.oneCell(&exec, allocator, "SELECT dbl FROM g WHERE x = 21");
        defer allocator.free(v21);
        try std.testing.expectEqualStrings("42", v21);
        const v5 = try h.oneCell(&exec, allocator, "SELECT dbl FROM g WHERE x = 5");
        defer allocator.free(v5);
        try std.testing.expectEqualStrings("10", v5);
    }

    // Session 2: reopen the SAME database. The column's generating expression is durable catalog
    // metadata (written to sys.tables, WAL-backed), so `loadCatalog` restores it: the reloaded
    // `dbl` column still carries `computed_by = "DBL(x)"`. That is the M4-specific persistence
    // claim, that the derived-data definition survives a restart. (Row-level data durability
    // across a plain close/reopen without an explicit checkpoint is a separate kaidb concern,
    // observed for ordinary tables too, e.g. the M2 UDF-persistence test.)
    {
        var db = try Database.open(allocator, io, db_path, 64, null);
        defer db.close();

        const tbl = db.catalog.getTable("g") orelse return error.TableNotReloaded;
        var found = false;
        for (tbl.columns) |c| {
            if (std.mem.eql(u8, c.name, "dbl")) {
                found = true;
                try std.testing.expect(c.computed_by != null);
                try std.testing.expectEqualStrings("DBL(x)", c.computed_by.?);
            }
        }
        try std.testing.expect(found);
    }

    // Session 3: prove the reloaded generating expression is live end to end. A brand-new table
    // in a fresh session has sound storage, so inserting into it exercises the full compute path
    // against the UDF that `Database.open` reloaded from disk, confirming the derived-data
    // definition is not just remembered but usable after a restart.
    {
        var db = try Database.open(allocator, io, db_path, 64, null);
        defer db.close();
        var exec = QueryExecutor.init(allocator, db);
        defer exec.deinit();
        try h.q(&exec, allocator, "CREATE TABLE g2 (x INT PRIMARY KEY, dbl INT AS DBL(x))");
        try h.q(&exec, allocator, "INSERT INTO g2 (x) VALUES (50)");
        const v50 = try h.oneCell(&exec, allocator, "SELECT dbl FROM g2 WHERE x = 50");
        defer allocator.free(v50);
