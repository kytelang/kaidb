...
test "parse tls 1.2 server hello" {
    ...
    h.cert = .{ .host = "example.ulfheim.net", .skip_verify = true, .root_ca = .{ .map = .empty, .bytes = .empty }, .now_sec = 0 };
    try h.readServerFlight1();
...
test "verify google.com certificate" {
    ...
    const io = testing.io;
    var ca_bundle: Certificate.Bundle = .{ .map = .empty, .bytes = .empty };
    try ca_bundle.rescan(testing.allocator, io, std.Io.Clock.real.now(io));
    defer ca_bundle.deinit(testing.allocator);

    h.cert = .{ .host = "google.com", .skip_verify = true, .root_ca = .{ .map = .empty, .bytes = .empty }, .now_sec = 1714846451 };
    try h.readServerFlight1();
    try h.verifyCertificateSignatureTls12();
}
...
test "nonblock handshake" {
    ...
    {
        try testing.expectEqual(data13.server_flight_1.len, res.recv_pos);
