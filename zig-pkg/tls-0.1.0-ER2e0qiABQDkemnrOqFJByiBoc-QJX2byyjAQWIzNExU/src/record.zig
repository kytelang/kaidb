...
test Decoder {
    ...
    try testing.expectEqual(45, try d.decode(u24));
    ...
    try testing.expectEqual(0, try d.decode(u8));
    try testing.expectEqual(.ECDHE_RSA_WITH_AES_128_CBC_SHA, try d.decode(CipherSuite));
    try testing.expectEqual(0, try d.decode(u8));
    try testing.expectEqual(5, try d.decode(u16));
...
pub const Writer = struct {
    ...
    pub fn serverName(w: *Writer, host: []const u8) !void {
        ...
        try w.int(u16, host_len + 5);
        try w.int(u16, host_len + 3);
        try w.inner.writeByte(0);
